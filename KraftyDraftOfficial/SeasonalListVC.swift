...
class SeasonalListVC: UIViewController, UITableViewDelegate, UITableViewDataSource, SeasonalModelProtocol, UISearchBarDelegate {
    
    var selectedSeasonal : SeasonalModel = SeasonalModel()
    let user = Auth.auth().currentUser
    let beerModel = SeasonalModel()
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        performSegue(withIdentifier: "SeasonalBeerDetail", sender: self)
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "SeasonalBeerDetail" {
...
extension SeasonalListVC : IndicatorInfoProvider {
    
    func indicatorInfo(for pagerTabStripController: PagerTabStripViewController) -> IndicatorInfo {
        return IndicatorInfo(title: "SEASONAL")
    }
}
