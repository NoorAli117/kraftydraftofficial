...
class RecentlyTappedListVC: UIViewController, UITableViewDelegate, UITableViewDataSource, RecentlyTappedModelProtocol, UISearchBarDelegate {
        
    var selectedRecently : RecentlyTappedModel = RecentlyTappedModel()
    let user = Auth.auth().currentUser
    let beerModel = RecentlyTappedModel()
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        performSegue(withIdentifier: "RecentBeerDetail", sender: self)
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "RecentBeerDetail" {
...
extension RecentlyTappedListVC : IndicatorInfoProvider {
    
    func indicatorInfo(for pagerTabStripController: PagerTabStripViewController) -> IndicatorInfo {
        return IndicatorInfo(title: "RECENTLY TAPPED")
    }
}
