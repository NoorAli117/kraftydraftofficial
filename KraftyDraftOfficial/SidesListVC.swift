...
class SidesListVC: UIViewController, UITableViewDelegate, UITableViewDataSource, SidesModelProtocol {
    
    var selectedSide : SidesModel = SidesModel()
...
//        tableView.estimatedRowHeight = 125.0
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        performSegue(withIdentifier: "SidesDetail", sender: self)
        
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "SidesDetail" {
...
extension SidesListVC : IndicatorInfoProvider {
    
    func indicatorInfo(for pagerTabStripController: PagerTabStripViewController) -> IndicatorInfo {
        return IndicatorInfo(title: "SIDES")
    }
}
