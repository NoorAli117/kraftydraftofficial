...
class LightListVC: UIViewController, UITableViewDelegate, UITableViewDataSource, LightModelProtocol, UISearchBarDelegate {
    
    var selectedLight : LightModel = LightModel()
    let user = Auth.auth().currentUser
    let beerModel = LightModel()
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        performSegue(withIdentifier: "LightBeerDetail", sender: self)
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "LightBeerDetail" {
...
extension LightListVC : IndicatorInfoProvider {
    
    func indicatorInfo(for pagerTabStripController: PagerTabStripViewController) -> IndicatorInfo {
        return IndicatorInfo(title: "LIGHT")
    }
}
