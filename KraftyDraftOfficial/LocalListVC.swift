...
class LocalListVC: UIViewController, UITableViewDelegate, UITableViewDataSource, LocalModelProtocol, UISearchBarDelegate {
    
    var selectedLocal : LocalModel = LocalModel()
    let user = Auth.auth().currentUser
    let beerModel = LocalModel()
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        performSegue(withIdentifier: "LocalBeerDetail", sender: self)
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "LocalBeerDetail" {
...
extension LocalListVC : IndicatorInfoProvider {
    
    func indicatorInfo(for pagerTabStripController: PagerTabStripViewController) -> IndicatorInfo {
        return IndicatorInfo(title: "LOCAL")
    }
}
