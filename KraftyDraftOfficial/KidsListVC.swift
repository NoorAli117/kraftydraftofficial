...
class KidsListVC: UIViewController, UITableViewDelegate, UITableViewDataSource, KidsModelProtocol {
    
    var selectedKid : KidsModel = KidsModel()
...
//        tableView.estimatedRowHeight = 125.0
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        performSegue(withIdentifier: "KidsDetail", sender: self)
        
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "KidsDetail" {
...
extension KidsListVC : IndicatorInfoProvider {
    
    func indicatorInfo(for pagerTabStripController: PagerTabStripViewController) -> IndicatorInfo {
        return IndicatorInfo(title: "KIDS")
    }
}
