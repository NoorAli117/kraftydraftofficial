...
class BeveragesListVC: UIViewController, UITableViewDelegate, UITableViewDataSource, BeveragesModelProtocol {
    
    var selectedBeverage : BeveragesModel = BeveragesModel()
...
//        tableView.estimatedRowHeight = 125.0
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        performSegue(withIdentifier: "BeveragesDetail", sender: self)
        
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "BeveragesDetail" {
...
extension BeveragesListVC : IndicatorInfoProvider {
    
    func indicatorInfo(for pagerTabStripController: PagerTabStripViewController) -> IndicatorInfo {
        return IndicatorInfo(title: "BEVERAGES")
    }
}
