...
class MaltyListVC: UIViewController, UITableViewDelegate, UITableViewDataSource, MaltyModelProtocol, UISearchBarDelegate {
    
    var selectedMalty : MaltyModel = MaltyModel()
    let user = Auth.auth().currentUser
    let beerModel = MaltyModel()
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        performSegue(withIdentifier: "MaltyBeerDetail", sender: self)
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "MaltyBeerDetail" {
...
extension MaltyListVC : IndicatorInfoProvider {
    
    func indicatorInfo(for pagerTabStripController: PagerTabStripViewController) -> IndicatorInfo {
        return IndicatorInfo(title: "MALTY")
    }
}
