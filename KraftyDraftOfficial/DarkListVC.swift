...
class DarkListVC: UIViewController, UITableViewDelegate, UITableViewDataSource, DarkModelProtocol, UISearchBarDelegate {
    
    var selectedDark : DarkModel = DarkModel()
    let user = Auth.auth().currentUser
    let beerModel = DarkModel()
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        performSegue(withIdentifier: "DarkBeerDetail", sender: self)
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "DarkBeerDetail" {
...
extension DarkListVC : IndicatorInfoProvider {
    
    func indicatorInfo(for pagerTabStripController: PagerTabStripViewController) -> IndicatorInfo {
        return IndicatorInfo(title: "DARK")
    }
}
