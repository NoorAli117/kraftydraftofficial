...
class FruityListVC: UIViewController, UITableViewDelegate, UITableViewDataSource, FruityModelProtocol, UISearchBarDelegate {
    
    var selectedFruity : FruityModel = FruityModel()
    let user = Auth.auth().currentUser
    let beerModel = FruityModel()
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        performSegue(withIdentifier: "FruityBeerDetail", sender: self)
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "FruityBeerDetail" {
...
extension FruityListVC : IndicatorInfoProvider {
    
    func indicatorInfo(for pagerTabStripController: PagerTabStripViewController) -> IndicatorInfo {
        return IndicatorInfo(title: "FRUITY")
    }
}
