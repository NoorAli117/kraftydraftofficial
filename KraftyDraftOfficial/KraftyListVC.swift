...
class KraftyListVC: UIViewController, UITableViewDelegate, UITableViewDataSource, KraftyModelProtocol, UISearchBarDelegate {
    
    var selectedKrafty : KraftyModel = KraftyModel()
    let user = Auth.auth().currentUser
    let beerModel = KraftyModel()
...
//            selectedTitle = self.beerItems[indexPath.row].fullBeerNames! + " - " + String(format: "%.01f", percent * 100) + "%"
        ...
        performSegue(withIdentifier: "KraftyBeerDetail", sender: self)
        
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "KraftyBeerDetail" {
...
extension KraftyListVC : IndicatorInfoProvider {
    
    func indicatorInfo(for pagerTabStripController: PagerTabStripViewController) -> IndicatorInfo {
        return IndicatorInfo(title: "KRAFTY")
    }
}
