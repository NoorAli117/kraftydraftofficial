...
class BucketListVC: UIViewController, UITableViewDelegate, UITableViewDataSource, BucketListModelProtocol {
    
    var selectedBucket : BucketListModel = BucketListModel()
    let user = Auth.auth().currentUser
    let beerModel = BucketListModel()
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        performSegue(withIdentifier: "BucketBeerDetail", sender: self)
        
    }
    
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "BucketBeerDetail" {
...
extension BucketListVC : IndicatorInfoProvider {
    
    func indicatorInfo(for pagerTabStripController: PagerTabStripViewController) -> IndicatorInfo {
        return IndicatorInfo(title: "BUCKET LIST")
    }
}
