...
class SoursListVC: UIViewController, UITableViewDelegate, UITableViewDataSource, SoursModelProtocol, UISearchBarDelegate {
    
    var selectedSours : SoursModel = SoursModel()
    let user = Auth.auth().currentUser
    let beerModel = SoursModel()
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        performSegue(withIdentifier: "SoursBeerDetail", sender: self)
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "SoursBeerDetail" {
...
extension SoursListVC : IndicatorInfoProvider {
    
    func indicatorInfo(for pagerTabStripController: PagerTabStripViewController) -> IndicatorInfo {
        return IndicatorInfo(title: "SOURS")
    }
}
